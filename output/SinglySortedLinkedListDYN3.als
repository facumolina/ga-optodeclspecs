...
(eq[(thiz) . (size_0),#((((thiz) . (header_0)) . (*(next_0))) - (Null))]) and (all n : ((thiz) . (header_0)) . (*(next_0)) | (n) !in ((n) . (^(next_0)))) and (all n : ((thiz) . (header_0)) . (*(next_0)) | (((n) . (next_0)) != (Null)) => (lte[(n) . (element_0),((n) . (next_0)) . (element_0)]))
}
