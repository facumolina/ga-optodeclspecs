...
(all n : one(((thiz) . (head)) . (*(next))) | !((n) in ((n) . (^(next))))) and (some(((thiz) . (head)) & (Node))) and (eq[(thiz) . (size),sub[#((((thiz) . (head)) . (*(next))) - (Null)),1]])
}
...
pred PositiveCounterExample[
   ...
   size: List -> one Int
	...
	SimmetryBreaking[thiz,head,next,size] and  repOKOK[thiz,head,next,size] and not catalog[thiz,head,next,size]
}
