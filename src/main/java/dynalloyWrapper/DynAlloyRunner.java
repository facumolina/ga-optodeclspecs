...
public class DynAlloyRunner {
	...
	/**
	 * Creates a DynAlloy runner object, associated to the provided file.
	 * repOK predicate name is simply repOK.
	 * @param filename is the name of the file containing the (Dyn)Alloy model
	 */
	...
	/**
	 * Creates a DynAlloy runner object, associated to the provided file, and a user
	 * provided predicate to be analyzed.
	 * @param filename is the name of the file containing the (Dyn)Alloy model
	 * @param repOkName is the name of the predicate for analysis (predicate containing)
	 * the invariant to be analyzed.
	 * @throws IOException 
	...
	/**
	 * @return ordered set of elements (starting with and's operands ) present in the repOK predicate as String
	 */
	...
	/**
	 * Set the body of the repOK predicate as a conjunction of elements which position is true in the parameter.
	 * 
	 * @param useElements: int array indicating if the element with index position will be present in
	 * the predicate body as an AND operand.
	 */
	...
	public boolean[] checkAsserts(){
		...
		for (Command cmd: commands) {
			// Execute the command
			try{
				A4Solution ans = TranslateAlloyToKodkod.execute_command(rep, ast.getAllReachableSigs(), cmd, options);
				//all command treated as check (if not so see cmd.check attribute)
				//set each result if ends as exprected 
				res[i]=  (cmd.expects==1 && ans.satisfiable() ) || (cmd.expects==0 && !ans.satisfiable());
				i++;
			} catch(Err e){
				System.out.print(e.msg);
			}
	 ...
	 * @throws Err 
	 */
	public double checkFailPercent(int assertnum){
		double res = 0.0;
		// Report and options to execute commands
		A4Options options = new A4Options();
		// Set the SatSolver
        options.solver = A4Options.SatSolver.SAT4J;
		A4Reporter rep = new A4Reporter();
		Command cmd = commands.get(assertnum);
		for (Expr elm: elements) {
			// Execute the command with only one element
			try{
				setRepOKbody(elm);
				A4Solution ans = TranslateAlloyToKodkod.execute_command(rep, ast.getAllReachableSigs(), cmd, options);
				if ( (cmd.expects==1 && ans.satisfiable() ) || (cmd.expects==0 && !ans.satisfiable())) res++;         
			} catch(Err e){
				System.out.print(e.msg);
			}
		}
	...
	/**
	 * Executes the field exhaustive approach in order to count the instances that satisfies and not the current repOK.
	 */
	private void executeFieldExahustive() throws IOException,Err,InterruptedException {
		...
		// Go to the field exhaustive directory and then call the python script to compute the instances.
		List<String> commands = new ArrayList<String>();
		commands.add("/bin/sh");
		commands.add("-c");
		commands.add("cd "+ConfigurationProperties.getFieldExhaustiveDirectory()+"; python2.7 bottomup-save-instances.py learning-spec/"+this.filename);
	...
	public void writeRepOkInFile(String currentFile) {
		...
		try {
			...
			commands.add("/bin/sh");
			commands.add("-c");
			commands.add(getSedCommand(currentRepOkString,currentFile+this.filename));
			ProcessBuilder pb = new ProcessBuilder(commands);
			Process process = pb.start();
			process.waitFor();
	...
	private void clearFieldExhaustiveDir() throws IOException, InterruptedException{
		...
		commands.add("/bin/sh");
		commands.add("-c");
		commands.add("rm "+ConfigurationProperties.getFieldExhaustiveDirectory()+"/instances/*");
		ProcessBuilder pb = new ProcessBuilder(commands);
		Process process = pb.start();
		process.waitFor();
		...
		commands.add("/bin/sh");
		commands.add("-c");
		commands.add("rm "+ConfigurationProperties.getFieldExhaustiveDirectory()+"/results/*");
	...
	private String getSedCommand(String currentRepOkString,String currentFile) {
		...
		if (currentOS.startsWith("Mac"))
			return "sed -i '' -e '"+currentRepOKLine+"s/.*/"+currentRepOkString+"/' "+currentFile;
	 ...
	 * @param value indicates the example type that will be generated. True for positive examples and false
	...
	public A4Solution generateExample(boolean value) {
		...
		for (Command cmd: commands) {
			// Execute the command
			try {
				if ((!value && cmd.label.equals("NegativeCounterExample"))|| (value && cmd.label.equals("PositiveCounterExample"))){
					A4Solution ans = TranslateAlloyToKodkod.execute_command(rep, ast.getAllReachableSigs(), cmd, options);
					if (ans.satisfiable()) {
						return ans;
					}
	...
	private void setUp(){
		...
		try{
			...
			// Get elements
			if (repOK == null){System.out.print("No repOK Found.");}
			if (catalog == null){
				System.out.print("No Catalog Found.");
				decls = repOK.decls;
   ...
   */
	public DataStructureInformation getStructureInformation() {
		return new DataStructureInformation(decls,commands);
	}
	...
	public Expr translateExpression(ExprVar expression) {
		for (Decl decl : decls) {
			for (ExprHasName expr: decl.names) {
				...
				if (expression.label.contains(declName)) {
