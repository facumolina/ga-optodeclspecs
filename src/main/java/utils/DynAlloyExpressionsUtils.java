...
/**
 * This class provides utilities for the DynAlloy expressions
 * management
 * @author fmolina
 */
public class DynAlloyExpressionsUtils {

	public static void printList(List<Chromosome> chromosomes) throws FileNotFoundException, UnsupportedEncodingException {
		PrintWriter writer = new PrintWriter("/home/koodu/facu/machinelearningspec/chromosomes.txt", "UTF-8");
	...
	/**
	 * Given a simple closured expression e.*f and a quantification operator
	 * creates the expression op n : e.*f : n != n.f
	 */	
	public static Expr createsQuantifiedExpressionCurrentAndNextNotEquals(Expr closuredExpression,ExprQt.Op op) {
		// Create n
	...
	/**
	 * Given a double closured expression e.*(f+g), a quantification operator and an int i
	 * creates the expression:
	 *  - op n : e.*(f+g) : n != n.f if i = 1
	 *  - op n : e.*(f+g) : n != n.g if i = 2
	 *  - op n : e.*(f+g) : n.f != n.g if i = 3
	 */	
	public static Expr createsQuantifiedExpressionCurrentAndNextNotEquals(Expr closuredExpression,ExprQt.Op op,int i) {
		// Create n 
		ExprVar n = ExprVar.make(null,"n",closuredExpression.type());
		List<ExprHasName> names = new LinkedList<ExprHasName>();
		names.add(n);
	...
	/**
	 * Given a double closured expression e.*(f+g), a joinable expression r, a possible value for r, a quantification operator and an int i
	 * creates the expression:
	 *  - op n : e.*(f+g) : n.r=v => n.f.r=v if i = 1
	 *  - op n : e.*(f+g) : n.r=v => n.g.r=v if i = 2
	 *  - op n : e.*(f+g) : n.r=v => n.f.r=v and n.g.r=v if i = 3
	 */
	public static Expr createsQuantifiedExpressionCurrentValueImpliesNextValueEqual(Expr closuredExpression,Expr joinableExpr,Type joinableExprReturnType,Expr value,ExprQt.Op op,int i) throws Err {
		// Create n
		ExprVar n = ExprVar.make(null,"n",closuredExpression.type());
		List<ExprHasName> names = new LinkedList<ExprHasName>();
		names.add(n);
		...
		Expr body;
		if (joinableExprReturnType.is_int()) {
			// Creates body with expr calls for int evaluation
			
			// Left equality. n.joinableExpr = value
			Expr leftSideFirst = ExprBinary.Op.JOIN.make(null, null, ExprUnary.Op.NOOP.make(null, n), ExprUnary.Op.NOOP.make(null, joinableExpr));
			...
			if (i==1 || i==2) {
				...
				Expr e = getExpressionAfterClosure(closuredExpression,i);
				Expr njoine = ExprBinary.Op.JOIN.make(null, null, n, e);
				Expr njoinejoinexpreqvalue = ExprBinary.Op.EQUALS.make(null, null, ExprBinary.Op.JOIN.make(null, null, njoine, joinableExpr), value);
				...
				Expr f = getExpressionAfterClosure(closuredExpression,1);
				Expr njoinf = ExprBinary.Op.JOIN.make(null, null, n, f);
				Expr njoinfjoinexpreqvalue = ExprBinary.Op.EQUALS.make(null, null, ExprBinary.Op.JOIN.make(null, null, njoinf, joinableExpr), value);
				
				// n.g.r=v 
				Expr g = getExpressionAfterClosure(closuredExpression,2);
				Expr njoing = ExprBinary.Op.JOIN.make(null, null, n, g);
				Expr njoingjoinexpreqvalue = ExprBinary.Op.EQUALS.make(null, null, ExprBinary.Op.JOIN.make(null, null, njoing, joinableExpr), value);
				
				// n.r=v => n.f.r=v and n.g.r=v 
				body = ExprBinary.Op.IMPLIES.make(null, null, njoinexpreqvalue, ExprBinary.Op.AND.make(null, null, njoinfjoinexpreqvalue, njoingjoinexpreqvalue));
			}
		}		
		// Create expression
		ExprQt qtExpr = (ExprQt) op.make(null, null,decls, body);
	 ...
	 */
	public static Expr createsQuantifiedExpressionCurrentValueOpNextValue(Expr closuredExpression,Expr joinableExpr,ExprQt.Op op,Sig nullSig) throws Err {
		// Create n
		ExprVar n = ExprVar.make(null,"n",closuredExpression.type());
		List<ExprHasName> names = new LinkedList<ExprHasName>();
		names.add(n);
		...
		Expr nJoinJoinableExpr = ExprBinary.Op.JOIN.make(null, null, ExprUnary.Op.NOOP.make(null, n), ExprUnary.Op.NOOP.make(null, joinableExpr));

		// Right equality n.f.joinableExpr
		Expr f = getExpressionAfterClosure(closuredExpression);
		Expr njoinf = ExprBinary.Op.JOIN.make(null, null, n, f);
		Expr njoinfjoinjoinabkeExpr = ExprBinary.Op.JOIN.make(null, null, ExprUnary.Op.NOOP.make(null, njoinf), ExprUnary.Op.NOOP.make(null, joinableExpr));
		
		// Creates args
		List<Expr> args = createsArgsForBinaryIntegerExprCall(nJoinJoinableExpr, njoinfjoinjoinabkeExpr);
		...
		body = ExprBinary.Op.IMPLIES.make(null, null, ExprBinary.Op.NOT_EQUALS.make(null, null, njoinf, nullSig), exprCall);
	 ...
	 */
	public static Expr createsQuantifiedExpressionCurrentValueOpFstNextValueCurrentValueOpSndNextValue(Expr closuredExpression,Expr joinableExpr,ExprQt.Op op,Sig nullSig) throws Err {
		// Create n
		ExprVar n = ExprVar.make(null,"n",closuredExpression.type());
		List<ExprHasName> names = new LinkedList<ExprHasName>();
		names.add(n);
		...
		Expr binaryLeft = ExprBinary.Op.IMPLIES.make(null, null, ExprBinary.Op.NOT_EQUALS.make(null, null, njoinf, nullSig), exprCall);
		
		// Second equality n.g.joinableExpr
		Expr g = getExpressionAfterClosure(closuredExpression,2);
		Expr njoing = ExprBinary.Op.JOIN.make(null, null, n, g);
		Expr njoingjoinjoinabkeExpr = ExprBinary.Op.JOIN.make(null, null, ExprUnary.Op.NOOP.make(null, njoing), ExprUnary.Op.NOOP.make(null, joinableExpr));
		
		// Creates args
		List<Expr> args2 = createsArgsForBinaryIntegerExprCall(nJoinJoinableExpr, njoingjoinjoinabkeExpr);
		...
		Expr binaryRight = ExprBinary.Op.IMPLIES.make(null, null, ExprBinary.Op.NOT_EQUALS.make(null, null, njoing, nullSig), exprCall2);
	...
	/**
	 * Given a double closured expression e.*(f+g), an expression r and a quantification operator creates a gene value with expression
	 * - op n : e.*(f+g) | (op m: n.f.*(f+g)-Null : n.r=m.r) AND (op m: n.g.*(f+g)-Null : n.r=m.r)
	 */
	public static Expr createsQuantifiedExpressionQtCurrentValueOpFstNextValueQtCurrentValueOpSndNextValue(Expr closuredExpression,Expr joinableExpr,ExprQt.Op op,Sig nullSig) throws Err {
		...
		Expr leftQtDomain = ExprBinary.Op.MINUS.make(null,null,ExprBinary.Op.JOIN.make(null, null, n, ExprBinary.Op.JOIN.make(null, null, plusexpr.left, ExprUnary.Op.RCLOSURE.make(null, plusexpr))),DataStructureInformation.nullSig);
		Expr rightQtDomain = ExprBinary.Op.MINUS.make(null, null, ExprBinary.Op.JOIN.make(null, null, n, ExprBinary.Op.JOIN.make(null, null, plusexpr.right, ExprUnary.Op.RCLOSURE.make(null, plusexpr))),DataStructureInformation.nullSig);
		...
		// m.joinableExpr
		Expr mJoinJoinableExpr = ExprBinary.Op.JOIN.make(null, null, ExprUnary.Op.NOOP.make(null, m), ExprUnary.Op.NOOP.make(null, joinableExpr));
		...
		// m.joinableExpr
		Expr oJoinJoinableExpr = ExprBinary.Op.JOIN.make(null, null, ExprUnary.Op.NOOP.make(null, o), ExprUnary.Op.NOOP.make(null, joinableExpr));
	...
	/**
	 * Given a simple closured expression e.*f and a quantification operator  
	 * creates the expression op n : e.*f : n in n.^f
	 */
	 ...
	 */
	public static Expr createsQuantifiedExpressionVarInSet(Expr closuredExpression,ExprQt.Op op,int i) {
		// Create variables
		ExprVar n = ExprVar.make(null,"n",closuredExpression.type());
		List<ExprHasName> names = new LinkedList<ExprHasName>();
		names.add(n);
	 ...
	 */
	public static Expr createsQuantifiedtExpressionSetNotEqualSet(Expr closuredExpression,ExprQt.Op op) {
		// Create variables
		ExprVar n = ExprVar.make(null,"n",closuredExpression.type());
		List<ExprHasName> names = new LinkedList<ExprHasName>();
		names.add(n);
	 ...
	 */
	public static Expr createsCardinalitySetEqualsToIntExpr(Expr closuredExpr,Expr intExpr) throws Err {
		Expr cardinality = ExprUnary.Op.CAST2SIGINT.make(null, ExprUnary.Op.CARDINALITY.make(null, ExprBinary.Op.MINUS.make(null, null, closuredExpr, DataStructureInformation.nullSig)));
	...
	/**
	 * Creates args that will be arguments of a binary expression between the given integer expressions
	 */
	 ...
	 */
	private static Func createsFunForIntegerComparisonExprCall(ExprVar n1,ExprVar n2,ExprBinary.Op op) throws Err {
		
		// Creates body
		Expr left = ExprUnary.Op.CAST2SIGINT.make(null, ExprUnary.Op.CAST2INT.make(null, ExprUnary.Op.NOOP.make(null, n1)));
		Expr right = ExprUnary.Op.CAST2SIGINT.make(null, ExprUnary.Op.CAST2INT.make(null, ExprUnary.Op.NOOP.make(null, n2)));
	...
	/**
	 * Given an equality between two int expressions e = f creates the expression
	 * e = f + 1 if add is true or the expression e = f - 1 if add is false 
	 */
	public static Expr addOrSubtractOneToIntComparisson(ExprCall intComparissonExpr,boolean add) throws Err {
	 ...
	 * expression e op' f.
	 */
	public static Expr changeOperatorInIntComparison(ExprCall intComparissonExpr,ExprBinary.Op op) throws Err {
		ExprVar n1 = ExprVar.make(null, "n1",Sig.SIGINT.type());
		ExprVar n2 = ExprVar.make(null, "n2",Sig.SIGINT.type());
	 ...
	 */
	private static ExprCall createsFunBodyForIntegerAdditionSubtraction(ExprVar n1,ExprVar n2,boolean add) throws Err {
		
		// Creates args
		List<Expr> args = createsArgsForBinaryIntegerExprCall(ExprUnary.Op.NOOP.make(null, n1),ExprUnary.Op.NOOP.make(null, n2));
	...
	/**
	 * Given a double closured expression e.*(f+g) and a var n, creates the expression n.f.*(f+g) != n.g.*(f+g) 
	 */
	private static Expr createsNotEqualsSetsBody(ExprVar n, Expr exprWithDoubleClosure) {
		// Search the expression (f+g) in exprWithDoubleClosure such that exprWithDoubleClosure has a subexpression e.*(f+g)
		...
		Expr firstExpr = ExprBinary.Op.JOIN.make(null, null, n, ExprBinary.Op.JOIN.make(null, null, plusexpr.left, ExprUnary.Op.RCLOSURE.make(null, plusexpr)));
		Expr secondExpr = ExprBinary.Op.JOIN.make(null, null, n, ExprBinary.Op.JOIN.make(null, null, plusexpr.right, ExprUnary.Op.RCLOSURE.make(null, plusexpr)));
	...
	/**
	 * Creates an expression n in n.^expr where exprWithClosure = e.*expr. This expression will be used 
	 * in the body of a quantified expression
	 * @param n
	 * @param exprWithClosure
	 * @return
	 */
	private static Expr createsInBody(ExprVar n, Expr exprWithClosure) {
		// Search the expression expr in exprWithClosure such that exprWithClosure has a subexpression e.*expr
	...
	/**
	 * Given a var n and a exprWithClosure = e.*(f+g) creates the expression
	 *  - n in n.^f if i = 1
	 *  - n in n.^g if i = 2
	 */
	...
	/**
	 * Given an expression e.*(f+g) and an int i, returns
	 * - f if i = 1
	 * - g if i = 2
	 */
	...
	public static Expr negateExpression(Expr expr) throws Err {
		if (expr instanceof ExprUnary) {
			...
			if (binaryBody.op.equals(ExprBinary.Op.EQUALS)) {
				...
				// The op is !=, so the new op is =
				Expr newBody = ExprUnary.Op.NOOP.make(null,ExprBinary.Op.EQUALS.make(null, null, binaryBody.left, binaryBody.right));
	...
	public static String getExpressionAsAlloyString(Expr expr) {
		if (expr instanceof Sig) {
			...
			if (exprUnary.op.equals(ExprUnary.Op.CAST2INT)||exprUnary.op.equals(ExprUnary.Op.CAST2SIGINT)||
			...
			return "("+getExpressionAsAlloyString(exprBinary.left)+") "+op+" ("+getExpressionAsAlloyString(exprBinary.right)+")";
		} else if (expr instanceof ExprCall) {
