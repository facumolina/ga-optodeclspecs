...
public class ExprGeneValue {
	 ...
	 * @param expr is the expression 
	 * @param geneType is the gene type associated with the given expression
	 * @param is the previous gene value from which the current was evolved
	 */
	...
	private ExprGeneType getGeneTypeFromExpression(Expr expr) {
		if (expr instanceof ExprConstant) {
			...
			if (exprBinary.op.equals(ExprBinary.Op.EQUALS)||exprBinary.op.equals(ExprBinary.Op.NOT_EQUALS)) {
	...
	/**
	 * Determine the gene type of a quantified expression which operator is all and the body
	...
	/**
	 * Determine the gene type of a quantified expression which operator is some and the body
	 * is the given expression
	 */
	...
	public String toString() {
		if (expression instanceof ExprQt) {
			...
			s += qt.sub.toString()+" - "+geneType.toString();
			return s;
		} else {
			return expression.toString() + " - " + geneType.toString();
		}
	}
	@Override
	public ExprGeneValue clone() {
		if (previous==null) {
			...
			return new ExprGeneValue(cloneExpression(expression),getGeneTypeFromExpression(expression),previous.clone());
		}
	...
	private Expr cloneExpression(Expr expr) {
		if (expr instanceof ExprConstant) {
			...
			ExprBinary exprBinary = (ExprBinary)expr;
			return exprBinary.op.make(null, null, cloneExpression(exprBinary.left),cloneExpression(exprBinary.right));
		} else if (expr instanceof ExprQt) {
			ExprQt exprQt = (ExprQt)expr;
			...
			return ExprBinary.Op.AND.make(null, null, cloneExpression(exprList.args.get(0)), cloneExpression(exprList.args.get(1)));
		}
