 ...
 * @author fmolina
 */
public class InvariantChromosome extends BaseChromosome {
	...
	/**
	 * Holds the counter examples of the invariant represented by this chromosome
	 */
	private Set<String> counterexamples;
	
	private Double amountOfCounterexamples;
	
	private boolean resultOfCrossoverOfEmtpyIntersectionChromosomes;
	
	/**
	 * Counterexamples type
	 *  0 : All the examples are negative
	 *  1 : All the examples are positive
	 *  2 : There are positive and negative examples
	 */
	private int counterexamplesType;
	
	/**
	 * Default constructor
	 * @throws InvalidConfigurationException
	 */
	public InvariantChromosome() throws InvalidConfigurationException {
		this(Genotype.getStaticConfiguration());
		counterexamples = new HashSet<String>();
		amountOfCounterexamples = new Double(-1);
	 ...
	 * @throws InvalidConfigurationException
	 */
	public InvariantChromosome(final Configuration a_configuration) throws InvalidConfigurationException {
	 ...
	 */
	public InvariantChromosome(final Configuration a_configuration, final int a_desiredSize) throws InvalidConfigurationException {
	 ...
	 */
	public InvariantChromosome(final Configuration a_configuration, Gene[] a_initialGenes) throws InvalidConfigurationException {
	...
	public synchronized Object clone() {
	    ...
	    try {
	      if (copy == null) {
	        ...
	        if (size > 0) {
	          ...
	          // ---------------------------------------------------------------
	          if (getClass() == InvariantChromosome.class) {
	            copy = new InvariantChromosome(getConfiguration(), copyOfGenes);
	          }
	          else {
	            copy = (InvariantChromosome) getConfiguration().getSampleChromosome().clone();
	            copy.setGenes(copyOfGenes);
	          }
	        }
	        else {
	          if (getClass() == InvariantChromosome.class) {
	            copy = new InvariantChromosome(getConfiguration());
	          }
	          else {
	            copy = (InvariantChromosome) getConfiguration().getSampleChromosome().clone();
	          }
	      ...
	      copy.setCounterexamplesType(getCounterexamplesType());
	    ...
	    if (m_multiObjective != null) {
	      if (getClass() == InvariantChromosome.class) {
	        try {
	          ( (InvariantChromosome) copy).setMultiObjectives( (List) cloneObject(
	              m_multiObjective));
	        } catch (Exception ex) {
	          throw new IllegalStateException(ex.getMessage());
	        }
	...
	/**
	 * IGeneConstraintChecker the constraint checker to be used whenever
	...
	public String toString() {
	    ...
	    representation.append("], Counterexamples: "+amountOfCounterexamples);
	    representation.append("]/");
	 ...
	 * genes values (alleles) randomized.
	 */
	public static IChromosome randomInitialChromosome( Configuration a_configuration) throws InvalidConfigurationException {
	    ...
	    // ---------------------------------------------------------
	    return new InvariantChromosome(a_configuration, newGenes);
	}
	
	/**
	 * Compares this Chromosome against the specified object. The result is
	 * true if and the argument is an instance of the InvariantChromosome class
	...
	/**
	 * Set the amount of counterexamples
	 */
	public void setAmountOfCounterexamples (Double amount) {
		if (amount < 0)
			throw new IllegalArgumentException();
		amountOfCounterexamples = amount;
	}
	
	/**
	 * Get the amount of couterexamples
	 */
	public Double getAmountOfCounterexamples () {
		return amountOfCounterexamples;
	}
	...
	/**
	 * Get the counterexamples type
	 */
	public int getCounterexamplesType() {
		return counterexamplesType;
	}
	
	/**
	 * Set the counterexamples type
	 */
	public void setCounterexamplesType(int type) {
		if ( type<0 || type>2)
			throw new IllegalArgumentException();
		counterexamplesType = type;
	}
	
	/**
	 * Get the counterexample set type
	 * @param stringCounterexamples
	 * @return
	 */
	private int getCounterexamplesSetType(Set<String> stringCounterexamples) {
		boolean existsPositives = false;
		boolean existsNegatives = false;
		for (String example : stringCounterexamples) {
			if (example.contains("Negative")) {
				existsNegatives = true;
			} else if (example.contains("Positive")) {
				existsPositives = true;
			}
			if (existsNegatives && existsPositives) {
				// There positive and negative examples
				return 2;
			}
		}
		if (existsNegatives) {
			// There are just negative examples
			return 0;
		} else if (existsPositives){
			// There are just positive examples
			return 1;
		} else {
			if (stringCounterexamples.isEmpty()) {
				return 0;
			} else {
				return -1;
			}
	...
	public String toStringAsAlloyExpression() {
		...
		for (int i=0; i < getGenes().length ; i++) {
			...
			if (!gene.isDefault()) {
				String currentExprString = DynAlloyExpressionsUtils.getExpressionAsAlloyString(gene.getValue().getExpression());
	...
	public void printGenes() {
		...
		System.out.print("]\nCOUNTEREXAMPLES: "+getAmountOfCounterexamples());
	}
	...
	/**
	 * Returns true if the chromosome is unary (the amount of active genes is one) and is valid (the fitness is greater than zero)
	 */
	public boolean isUnaryAndValid() {
		return getAmountOfActiveGenes()==1 && getFitnessValueDirectly()>0;
	}
